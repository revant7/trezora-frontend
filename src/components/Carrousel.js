...
export default function Category() {
    ...
    return (
        <div className="relative overflow-hidden w-full max-w-screen mx-auto max-h-80 h-80">
            <div
                ref={carouselRef}
            ...
            >
                {categories.map((item, index) => (
                    <div key={index} className="w-full flex-shrink-0" style={{ width: "100%" }}>
                        <img src={`/images/${item}`} alt={item} className="w-full h-64 object-cover rounded-lg shadow-md" />
                    </div>
                ))}
            </div>
